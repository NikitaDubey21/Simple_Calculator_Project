...
// Project:   Calculator
// Author:    Nikita Dubey
// Module description:  4 function single positive digit calculator 
// ***********************************************************
...
sbit RS    = P3^0;     //RS pin for LCD
sbit E   = P3^1;     //E pin for LCD
...
int main(void)
{
   ...
   float num1 = 0;                 //First number
   char func;              //Function to be performed among two numbers
   float num2 = 0;                 //Second number
	 float r;
	 int s,t;
   cct_init();                   //Make input and output pins as required
   lcdinit();                    //Initilize LCD
	 writeline("  <Calculator> ");  //To Display title on lCD
		
   while(1)
   { 
     //get numb1
     key = get_key();
     writecmd(0xC0);            //clear display
...
int get_num(char ch)         //convert char into int
{
	switch(ch)
	{
		...
		case ' ': writeline("                "); return Error; break;  //this is used as a clear screen and then reset by setting error
